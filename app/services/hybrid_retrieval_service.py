...
def bm25_search(query: str, top_k: int = 10) -> list:
    ...
    corpus = [c["content"].lower().split() for c in chunk_store]
    ...
    return [chunk_store[i] for i in top_indices]
...
def hybrid_search(query: str, query_embedding: list, top_k: int = 5) -> list:
    ...
    semantic_results = search_similar_chunks(query_embedding, top_k=fetch_k)
